...
void Forest::predict() {
  ...
  aborted_threads = 0;
#endif
  // Split the data into data set
  // Predict
  ret = sched_setattr(0, &attr, flags);

  if (ret < 0) {
     std::cout << "can't set deadline attr" << std::endl;
     return;
  } else {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    /*
    char * ptr;
    ptr = (char *) malloc(sizeof(char) * 16 * 1024);
    for (int i = 0 ; i < 16 * 1024; i++) {
      ptr[i] = i;
    }*/

    clock_gettime(CLOCK_REALTIME, &start);
    predictTreesInThread(0, data.get(), false);
  /*
  std::vector<std::thread> threads;
  threads.reserve(num_threads);
  for (uint i = 0; i < num_threads; ++i) {
  ...
  */
  ...
  for (auto &thread : threads) {
    ...
    std::cout << "interval: " << temp.tv_sec << " sec, " << temp.tv_nsec << " nsec" << std::endl;
    if (((temp.tv_nsec >> 10) + runtime) > 30000) {
      runtime = (temp.tv_nsec >> 10) + runtime - 30000;
      miss_count += 1;
      std::cout << "miss" << std::endl;
    } else {
      unsigned long sleep_time = 31000 - (temp.tv_nsec >> 10) - runtime;
      usleep(sleep_time);
      runtime = 0;
    }
  }
  std::cout << "miss ratio : " << miss_count/10.0 << "%" << std::endl;
#ifdef R_BUILD
  if (aborted_threads > 0) {
    throw std::runtime_error("User interrupt.");
  }
...
void Forest::predictTreesInThread(uint thread_idx, const Data* prediction_data, bool oob_prediction) {
  ...
  struct timespec temp = diff(start_time, cur);
  /*
